...
"""
my_bvsb_clsbal_v2.py
: Best versus second Best (BvsB) selector + Class balancing
...
class RegionSelector(my_bvsb.RegionSelector):
    ...
    def calculate_scores(self, trainer, pool_set):
        ...
        with torch.no_grad():
            ...
            dominant_nclass = F.one_hot(dominant_class, num_classes= (self.num_class)) # (N x self.num_superpixels, C+1)
            est_dominant_label_dist = dominant_nclass.sum(dim=0) / dominant_nclass.sum()
            cls_weight = torch.exp(-est_dominant_label_dist) # (C+1,)
