...
from . import region_cityscapes_dominant_all

class RegionCityscapesDominantAll(region_cityscapes_dominant_all.RegionCityscapesDominantAll):
    ...
    def __init__(self, args, root, datalist, split='train', transform=None, return_spx=False,
        ...
        super().__init__(args, root, datalist, split, transform, return_spx, region_dict, mask_region, dominant_labeling, or_labeling, generate_ignore)
    ...
    def __getitem__(self, index):
        ...
        for p in preserving_labels:
            ...
            if c.size != 0:
                ### assign probabilistic label
                selected_cls_idx = torch.multinomial(torch.Tensor(c), num_samples=1, replacement=False).item()
                target[sp_mask] = u[selected_cls_idx]
        if self.generate_ignore:
            pass
        else:
