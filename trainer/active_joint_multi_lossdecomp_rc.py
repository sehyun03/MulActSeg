...
from trainer import active_joint_multi_lossdecomp
from utils.loss import MultiChoiceCE
r"""
Applying naive top-1 selection loss to partial labels
"""

class OnehotCEMultihotRC(MultiChoiceCE):
    ...
    def forward(self, inputs, targets, superpixels, spmasks):
        ...
        for i in range(N):
            ...
            pos_pred_ = (valid_output * trg_pixel).sum(dim=1)
            pos_pred = pos_pred_.sum(dim=1)
            ...
            pos_pred_mh = pos_pred_[multihot_trg]
            if torch.any(multihot_trg):
                # assert(torch.all(multihot_trg == (1 < trg_pixel.sum(dim=1))))
                with torch.no_grad():
                    pos_preds_mh_ = pos_pred_mh.detach()
                    pos_weight = pos_preds_mh_ / pos_preds_mh_.sum(dim=1)[..., None] ### (N' x C): normalized pred as weight
                loss = -torch.log(pos_pred_mh + self.eps)
                mh_loss += (loss * pos_weight).sum()
                mh_num_valid += pos_pred_mh.shape[0]

        return oh_loss / oh_num_valid, mh_loss / mh_num_valid

class ActiveTrainer(active_joint_multi_lossdecomp.ActiveTrainer):
    def __init__(self, args, logger, selection_iter):
        super().__init__(args, logger, selection_iter)

    def get_criterion(self):
        ''' Define criterion '''
        self.multi_pos_loss = OnehotCEMultihotRC(num_class=self.num_classes, temperature=self.args.multi_ce_temp)
    ...
    def train_impl(self, total_itrs, val_period):
        ...
        for iteration in pbar:
            ...
            loss = (self.args.coeff * ce_loss) + (self.args.coeff_mc * mc_loss)
