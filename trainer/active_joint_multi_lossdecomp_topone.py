...
class OnehotCEMultihotTopone(MultiChoiceCE):
    ...
    def forward(self, inputs, targets, superpixels, spmasks):
        ...
        for i in range(N):
            ...
            if torch.any(multihot_trg):
                ...
                mh_top_pred = (valid_output * trg_pixel)[multihot_trg].max(dim=1)[0]
                mh_loss += -torch.log(mh_top_pred + self.eps).sum()
                mh_num_valid += mh_top_pred.shape[0]
...
class ActiveTrainer(active_joint_multi_lossdecomp.ActiveTrainer):
    ...
    def get_criterion(self):
        ''' Define criterion '''
        self.multi_pos_loss = OnehotCEMultihotTopone(num_class=self.num_classes, temperature=self.args.multi_ce_temp)
