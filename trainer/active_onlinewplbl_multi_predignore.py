...
class LocalWProtoCE(LocalProtoCE):
    ...
    def generate_plbl(self, inputs_plbl, feats_plbl, targets, superpixels, spmasks):
        ...
        for i in range(N):
            ...
            if self.args.weight_wo_proto:
                weight[i, validex_to_pixdex[top1_vdx]] = 1.0
...
class ActiveTrainer(active_onlineplbl_multi_predignore.ActiveTrainer):
    ...
    def get_criterion(self):
        ''' Define criterion '''
        self.multi_local_proto_loss = LocalWProtoCE(args=self.args, num_superpixel=self.args.nseg, temperature=self.args.group_ce_temp)
