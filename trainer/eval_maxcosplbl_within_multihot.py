...
r''''
Pseudo labeling using maximum similarity between local and global (classifier) prototypes
'''
...
class ActiveTrainer(ActiveTrainer):
    ...
    def pseudo_label_generation(self, labels, feats, inputs, targets, spmasks, superpixels):
        ...
        inputs = inputs.permute(0,2,3,1).reshape(N, -1, C)
        ...
        for i in range(N):
            ...
            if not torch.any(valid_mask): ### valid pixel 이 하나도 없으면 loss 계산 X
                continue #TODO
            valid_input = inputs[i][valid_mask]
            ...
            nn_local_similarity = mxproto_pxl.T[torch.arange(vspdex_superpixel.shape[0]), vspdex_superpixel]

            trg_pixel = multi_hot_target[vpx_superpixel.squeeze(dim=1)]
            nn_global_similarity, nn_global_cls = (valid_input * trg_pixel).max(dim=1)
            print((nn_local_similarity < nn_global_similarity).sum())

            plbl_vdx = vcdex[nn_local_cls]
