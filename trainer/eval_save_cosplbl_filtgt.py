...
class ActiveTrainer(ActiveTrainer):
    ...
    def pseudo_label_generation(self, labels, feats, inputs, targets, spmasks, superpixels):
        ...
        for i in range(N):
            ...
            r''' Pseudo label filtering '''
            model_prediction = outputs[i, validex_to_pixdex].argmax(dim=1)
            prediction_and_mask = (model_prediction == plbl_vdx)
            validex_to_pixdex_filt = validex_to_pixdex[prediction_and_mask]
            plbl_vdx_filt = plbl_vdx[prediction_and_mask]

            nn_plbl[i, validex_to_pixdex_filt] = plbl_vdx_filt
            nn_plbl[i, validex_to_pixdex[top1_vdx]] = vcdex
