...
from trainer.eval_within_multihot_voc import ActiveTrainer
from models import get_model, freeze_bn
from utils.miou import MeanIoU
r'''
- Pseudo label with naive argmax prediction
'''
...
class ActiveTrainer(ActiveTrainer):
    ...
    def inference(self, loader, prefix=''):
        ...
        if args.plbl_type is not None:
            ...
            save_dir = '{}/plbl_gen_ms/round_{}'.format(checkpoint_dir, round)
        ...
        with torch.no_grad():
            for iteration in trange(N):
                ...
                im_size = batch['imsizes'][0][::-1] ### im_size: height, width *PIL.size:: width, height
                fname = batch['fnames'][0][1]
                feat_list = []
                output_list = []
                for msidx, img in enumerate(batch['image_list'][0]):
                    img = img.to(self.device, dtype=torch.float32)
                    feat, output = self.net.feat_forward(img[None, ...])
                    ...
                    feat = tF.resize(feat, im_size, InterpolationMode.BILINEAR)
                    output = tF.resize(output, im_size, InterpolationMode.BILINEAR)

                    feat_list.append(feat[0])
                    output_list.append(output[0])

                feats = torch.stack(feat_list).mean(dim=0)
                outputs = torch.stack(output_list).mean(dim=0)
                ...
                r''' argmax based pseudo label acquisition '''
                ...
                if args.save_vis:
                    ...
                    vis_nn_plbl = torch.masked_fill(argmax_pseudo_label[0], argmax_pseudo_label[0]==255, 21).cpu()
                    ...
                    vispil_plbl_save = Image.fromarray(vis_nn_plbl)
