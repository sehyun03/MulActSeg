...
from utils.sliding_evaluator_plbl import SlidingEval
...
class ActiveTrainer(ActiveTrainer):
    ...
    def inference(self, loader, prefix=''):
        ...
        self.evaluator = SlidingEval(model=self.net,
                                     ...
                                     stride_rate=2/3,
                                     device="cuda:0",
                                     class_number=self.num_classes+1,
                                     val_id=1)
        ...
        with torch.no_grad():
            for iteration in trange(N):
                ...
                feats, outputs = self.evaluator(img=images)
                feats = torch.from_numpy(feats).to(device=self.device, non_blocking=True)[None, ...]
                outputs = torch.from_numpy(outputs).to(device=self.device, non_blocking=True)[None, ...]
