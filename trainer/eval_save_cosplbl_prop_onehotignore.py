...
- - This code is for label expansion ablation study with dominant labeling
- - This is sligh modification with predignore option
'''
...
class ActiveTrainer(ActiveTrainer):
    ...
    def get_al_model(self):
        args = self.args
        net = get_model(model=args.model, num_classes=self.num_classes + 1,
    ...
    def inference(self, loader, prefix=''):
        ...
        with torch.no_grad():
            for iteration in trange(N):
                ...
                spmasks = (targets != 255) ###spmask 를 255 아닌 영역으로 업데이트 (255 는 expansion X)

                ### decode targets into superpixel-wise one-hot vector
                r'''
                - scatter max 로 encode
                - mask_fill 로 255 를 19으로 변환
                - onehot function 으로 expansion => 마지막 차원 날리기
                '''

                spx_cls_label, _ = scatter_max(targets[0].view(-1), superpixels[0].view(-1))
                spx_cls_label_= torch.masked_fill(spx_cls_label, spx_cls_label == 255, 19)
                spx_cls_onehot = torch.nn.functional.one_hot(spx_cls_label_, num_classes=20)
                targets = spx_cls_onehot[None, ...]
