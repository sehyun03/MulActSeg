...
class ActiveTrainer(ActiveTrainer):
    ...
    def pseudo_label_generation(self, labels, feats, inputs, targets, spmasks, superpixels):
        ...
        is_trg_onehot = (1 == targets.sum(dim=2))
        ...
        ### addiitonally save single labeled regions
        for i in range(N):
            r''' filtered outputs '''
            if not torch.any(spmasks[i]): continue
            validall_superpixel = superpixels[i][spmasks[i]]
            onehot_mask = is_trg_onehot[i][validall_superpixel.squeeze(dim=1)].detach()
            valid_mask = spmasks[i].clone()
            valid_mask[spmasks[i]] = onehot_mask
            if not torch.any(valid_mask): continue

            vpx_superpixel = superpixels[i][valid_mask] ### HW' x 1
            trg_sup = targets[i]
            trg_pixel = trg_sup[vpx_superpixel.squeeze(dim=1)]
            trg_plbl_pixel = torch.argmax(trg_pixel, dim=1)
            validex_to_pixdex = valid_mask.nonzero().squeeze(dim=1)
            nn_plbl[i, validex_to_pixdex] = trg_plbl_pixel
