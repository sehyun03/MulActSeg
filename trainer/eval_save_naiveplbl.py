...
r""" Generate (save) pseudo label using naive top-1
"""
    ...
    def inference(self, loader, prefix=''):
        ...
        with torch.no_grad():
            for iteration in trange(N):
                ...
                r''' naive top-1 pseudo label acquisition with pseudo label thresholding'''
                if args.plbl_th > 0:
                    pred_prob = torch.softmax(outputs, dim=1)
                    spmasks = pred_prob.max(dim=1)[0] > args.plbl_th
                naive_pseudo_label = outputs.max(dim=1)[1]
                naive_pseudo_label = torch.masked_fill(naive_pseudo_label, torch.logical_not(spmasks), 255)
                ### ㄴ N x H x W

                output_dict = {
                    'outputs': naive_pseudo_label,
                    'targets': labels
                }
                ...
                plbl_save = naive_pseudo_label[0].cpu().numpy().astype('uint8')
                ...
                if args.save_vis:
                    ...
                    vis_nn_plbl = torch.masked_fill(naive_pseudo_label[0], naive_pseudo_label[0]==255, 20).cpu()
